...
@Service
public class ChildrenDepartmentRequestService {
    private final ChildrenDepartmentRequestRepository requestRepository;
    private final MembersRepository membersRepository;
    private final TeamsRepository teamsRepository;
    private final TeamMembersRepository teamMembersRepository;

    @Autowired
    public ChildrenDepartmentRequestService(
            ChildrenDepartmentRequestRepository requestRepository,
    ...
    public ChildrenDepartmentRequest createRequest(ChildrenDepartmentRequest request) {
        // Validation
        ...
        Optional<ChildrenDepartmentRequest> existingByPhone = requestRepository.findByPhoneAndContacted(phone, false);
        Optional<ChildrenDepartmentRequest> existingByEmail = requestRepository.findByEmailAndContacted(email, false);
    ...
    public List<ChildrenDepartmentRequest> getAllRequests() {
        return requestRepository.findAllByOrderByRequestDateDesc();
    }

    public List<ChildrenDepartmentRequest> getUncontactedRequests() {
        return requestRepository.findByContacted(false);
    }

    public ChildrenDepartmentRequest markAsContacted(Long id) {
        ChildrenDepartmentRequest request = requestRepository.findById(id)
    ...
    public String addToMembersList(Long requestId) {
        // 1. Get request
        ChildrenDepartmentRequest request = requestRepository.findById(requestId)
                .orElseThrow(() -> new RuntimeException("Request not found"));
        ...
        // 4. Find Children Department team (should already exist)
        Teams childrenTeam = teamsRepository.findByTeam_nameIgnoreCase("Children Department")
                .or(() -> teamsRepository.findByTeam_nameIgnoreCase("Children"))
                .orElseThrow(() -> new RuntimeException("Children Department team not found. Please create it first."));

        // 5. Check if member is already in the team
        if (!teamMembersRepository.existsByTeamsAndMembers(childrenTeam, member)) {
            ...
            TeamMembers teamMember = new TeamMembers(childrenTeam, member, joinedDate);
        ...
        return "Member added to Children Department successfully";
    }
}
