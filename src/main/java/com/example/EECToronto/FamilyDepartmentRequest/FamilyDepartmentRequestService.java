...
@Service
public class FamilyDepartmentRequestService {
    private final FamilyDepartmentRequestRepository requestRepository;
    private final MembersRepository membersRepository;
    private final TeamsRepository teamsRepository;
    private final TeamMembersRepository teamMembersRepository;

    @Autowired
    public FamilyDepartmentRequestService(
            FamilyDepartmentRequestRepository requestRepository,
    ...
    public FamilyDepartmentRequest createRequest(FamilyDepartmentRequest request) {
        // Validation
        ...
        Optional<FamilyDepartmentRequest> existingByPhone = requestRepository.findByPhoneAndContacted(phone, false);
        Optional<FamilyDepartmentRequest> existingByEmail = requestRepository.findByEmailAndContacted(email, false);
    ...
    public List<FamilyDepartmentRequest> getAllRequests() {
        return requestRepository.findAllByOrderByRequestDateDesc();
    }

    public List<FamilyDepartmentRequest> getUncontactedRequests() {
        return requestRepository.findByContacted(false);
    }

    public FamilyDepartmentRequest markAsContacted(Long id) {
        FamilyDepartmentRequest request = requestRepository.findById(id)
    ...
    public String addToMembersList(Long requestId) {
        // 1. Get request
        FamilyDepartmentRequest request = requestRepository.findById(requestId)
                .orElseThrow(() -> new RuntimeException("Request not found"));
        ...
        // 4. Find Family Department team (should already exist)
        Teams familyTeam = teamsRepository.findByTeam_nameIgnoreCase("Family Department")
                .or(() -> teamsRepository.findByTeam_nameIgnoreCase("Family"))
                .orElseThrow(() -> new RuntimeException("Family Department team not found. Please create it first."));
        ...
        return "Member added to Family Department successfully";
    }
}
