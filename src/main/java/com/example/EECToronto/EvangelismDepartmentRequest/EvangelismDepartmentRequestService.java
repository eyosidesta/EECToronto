...
@Service
public class EvangelismDepartmentRequestService {
    private final EvangelismDepartmentRequestRepository requestRepository;
    private final MembersRepository membersRepository;
    private final TeamsRepository teamsRepository;
    private final TeamMembersRepository teamMembersRepository;

    @Autowired
    public EvangelismDepartmentRequestService(
            EvangelismDepartmentRequestRepository requestRepository,
    ...
    public EvangelismDepartmentRequest createRequest(EvangelismDepartmentRequest request) {
        // Validation
        ...
        Optional<EvangelismDepartmentRequest> existingByPhone = requestRepository.findByPhoneAndContacted(phone, false);
        Optional<EvangelismDepartmentRequest> existingByEmail = requestRepository.findByEmailAndContacted(email, false);
    ...
    public List<EvangelismDepartmentRequest> getAllRequests() {
        return requestRepository.findAllByOrderByRequestDateDesc();
    }

    public List<EvangelismDepartmentRequest> getUncontactedRequests() {
        return requestRepository.findByContacted(false);
    }

    public EvangelismDepartmentRequest markAsContacted(Long id) {
        EvangelismDepartmentRequest request = requestRepository.findById(id)
    ...
    public String addToMembersList(Long requestId) {
        // 1. Get request
        EvangelismDepartmentRequest request = requestRepository.findById(requestId)
                .orElseThrow(() -> new RuntimeException("Request not found"));
        ...
        // 4. Find Evangelism Department team (should already exist)
        Teams evangelismTeam = teamsRepository.findByTeam_nameIgnoreCase("Evangelism Department")
                .or(() -> teamsRepository.findByTeam_nameIgnoreCase("Evangelism"))
                .orElseThrow(() -> new RuntimeException("Evangelism Department team not found. Please create it first."));

        // 5. Check if member is already in the team
        if (!teamMembersRepository.existsByTeamsAndMembers(evangelismTeam, member)) {
            ...
            TeamMembers teamMember = new TeamMembers(evangelismTeam, member, joinedDate);
        ...
        return "Member added to Evangelism Department successfully";
    }
}
